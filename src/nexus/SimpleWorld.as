package nexus{
	...
	public class SimpleWorld extends World {
		...
		public function SimpleWorld(stage:Stage, spriteSheet:DisplayObject, spritesPosition:String, spotLight:Boolean = false, normalMap:DisplayObject=null, backgroundColor:uint = 0xCCCCCC) {
			super(stage.stageWidth, stage.stageHeight, stage.frameRate);
			super.initialize(stage, { antiAliasing:0, mipmap:false, backColor:backgroundColor } )
		...
		public function render(lightPoint:Point = null, color:uint = 0xFFFFFF):void {
			super.renderStageToPass(_firstPass, false);
			if(_normalOn){
				super.renderStageToPass(_normPass, true);
				if(_spot){
					super.renderTextureToPass(_firstPass.getTexture(), _normPass.getTexture(),
					_showPass, { fragment:new<Number>[lightPoint.x/_w, lightPoint.y/_h, _lightSize, _h/_w],
					fragment2:new <Number>[0.1, 1, 10, 0.5]})
				}else {
					var r:Number = (color >> 16) * 0.00390625;
					var g:Number = (color >> 8 & 0xFF) * 0.00390625;
					var b:Number = (color & 0xFF) * 0.00390625;
					super.renderTextureToPass(_firstPass.getTexture(), _normPass.getTexture(),
					_showPass, { fragment:new<Number>[lightPoint.x, lightPoint.y, _lightSize, _h/_w],
					fragment2:new <Number>[r, g, b, 1]})
				}
			}else {
